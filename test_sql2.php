...
(SELECT COALESCE(SUM(f.total_ht), 0)
    ...
    WHERE el1.targettype = 'shipping' AND el1.fk_target = e.rowid) as pt1,
(SELECT COALESCE(SUM(f.total_ht), 0)
    FROM ".MAIN_DB_PREFIX."element_element el
    JOIN ".MAIN_DB_PREFIX."facture f ON el.fk_target = f.rowid AND el.targettype = 'facture'
    WHERE el.sourcetype = 'shipping' AND el.fk_source = e.rowid) as pt2
FROM ".MAIN_DB_PREFIX."expedition e
";
$res = $db->query($sql);
if ($res) {
    while($row = $db->fetch_object($res)) {
        echo $row->ref . ": SH=" . $row->expedition_amount_ht . " pt1=" . $row->pt1 . " pt2=" . $row->pt2 . "\n";
    }
} else {
    echo $db->lasterror();
}
