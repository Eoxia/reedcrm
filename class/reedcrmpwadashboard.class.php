 ...
 */
class ReedcrmPWADashboard extends SaturneDashboard
{
    /**
     * Constructor.
     *
	...
	public function load_dashboard(?array $moreParams = []): array
	{
        ...
        if (empty($dashboardConfig->graphs->OpportunitySources->hide)) {
            $array['graphs'][] = self::getOpportunitySources('OpportunitySources');
        } else {
            $array['disabledGraphs']['OpportunitySources'] = $langs->transnoentities('OpportunitySources');
        }

        if (empty($dashboardConfig->graphs->PropalStatusCommRepartition->hide)) {
            $array['graphs'][] = self::getDataFromExtrafieldsAndDictionary('PropalStatusCommRepartition', 'c_commercial_status');
        } else {
            $array['disabledGraphs']['PropalStatusCommRepartition'] = $langs->transnoentities('PropalStatusCommRepartition');
        }
        if (empty($dashboardConfig->graphs->PropalRefusalReasonRepartition->hide)) {
            $array['graphs'][] = self::getDataFromExtrafieldsAndDictionary('PropalRefusalReasonRepartition', 'c_refusal_reason', 'commrefusal');
        } else {
            $array['disabledGraphs']['PropalRefusalReasonRepartition'] = $langs->transnoentities('PropalRefusalReasonRepartition');
        }
