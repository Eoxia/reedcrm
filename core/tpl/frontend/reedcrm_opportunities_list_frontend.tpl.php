...
if (!empty($latestProjects) && is_array($latestProjects)) {
    // Add translation if not exists or fallback
    $listTitle = $langs->trans('LatestCreatedOpportunities');
    if ($listTitle == 'LatestCreatedOpportunities') {
        $listTitle = 'Dernières opportunités créées';
    }
    ...
    print '<style>
        ...
        .minimal-audio::-webkit-media-controls-volume-slider-container,
        .minimal-audio::-webkit-media-controls-volume-slider,
        .minimal-audio::-webkit-media-controls-mute-button,
        .minimal-audio::-webkit-media-controls-panel-menu-button { display: none !important; }
        .minimal-audio::-webkit-media-controls-enclosure { border-radius: 20px; background-color: #f1f3f4; }
        .minimal-audio::-webkit-media-controls-time-remaining-display { display: flex !important; }
    </style>';

    foreach ($latestProjects as $project) {
        // If the query didn't fetch full object properties, we hydrate them
        if (empty($project->date_c) && empty($project->datec)) {
            $tmpObj = new Project($db);
            $tmpObj->fetch($project->id);
            $project->date_c = $tmpObj->date_c;
            $project->datec = $tmpObj->datec;
            $project->user_author_id = $tmpObj->user_author_id;
            $project->fk_user_creat = $tmpObj->fk_user_creat;
        }
